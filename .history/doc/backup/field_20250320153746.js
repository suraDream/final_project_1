  ...
  try {
    ...
    const result = await pool.query(
      `SELECT 
          ...
          f.number_bank, f.account_holder, f.status, f.price_deposit,open_days
...
router.put("/:field_id", async (req, res) => {
  try {
    ...
    // ถ้าสถานะเป็น "ผ่านการอนุมัติ" ให้เปลี่ยน role ของผู้ใช้งานเป็น "field owner"
    ...
    const result = await pool.query(
      `UPDATE field 
       SET field_name = COALESCE($1, field_name), 
           ...
           status = COALESCE($10, status)  -- อัปเดตสถานะ
       WHERE field_id = $11 
       RETURNING *;`,
