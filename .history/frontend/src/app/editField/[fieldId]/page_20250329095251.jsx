...
export default function CheckFieldDetail() {
  ...
  try {
    ...
    const subFieldQuery = await pool.query(
      "SELECT * FROM sub_field WHERE main_field_id = $1",
      [mainFieldId]
    );

    if (subFieldQuery.rows.length > 0) {
      // 2. ลบสนามย่อยทั้งหมดที่เกี่ยวข้องกับสนามหลักนี้
      await pool.query("DELETE FROM sub_field WHERE main_field_id = $1", [
        mainFieldId,
      ]);
    }

    // 3. ลบสนามหลัก
    await pool.query("DELETE FROM main_field WHERE main_field_id = $1", [
      mainFieldId,
    ]);
