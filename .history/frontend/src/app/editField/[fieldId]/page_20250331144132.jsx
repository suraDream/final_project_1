...
export default function CheckFieldDetail() {
  ...
  useEffect(() => {
    ...
    if (user.role === "admin") {
      // admin สามารถเข้าถึงได้ทุกฟิลด์
      setIsLoading(false);
    } else if (user.role === "field_owner" && user.field_id === fieldId) {
      // ตรวจสอบว่า fieldId ของผู้ใช้ตรงกับ fieldId ที่กำหนดหรือไม่
      setIsLoading(false);
    } else {
      // หาก role เป็น field_owner แต่ไม่ตรงกับ fieldId จะไม่ให้เข้าถึง
      setMessage("คุณไม่มีสิทธิ์เข้าถึงข้อมูลนี้");
      setMessageType("error");
      router.push("/"); // ส่งกลับไปยังหน้าหลัก
    }
  }, [fieldId, router]);
