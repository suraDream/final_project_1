...
export default function CheckFieldDetail() {
  ...
  const { fieldId } = useParams();
  console.log("Field ID from URL:", fieldId);
  const deleteMainField = async (fieldId) => {
    // Handle possible array of fieldId
    const mainFieldId = Array.isArray(fieldId) ? fieldId[0] : fieldId;
  
    // Check if mainFieldId is valid
    if (!mainFieldId || isNaN(mainFieldId)) {
      alert("❌ ID ของสนามหลักไม่ถูกต้อง");
      return;
    }
  
    console.log("Main Field ID:", mainFieldId); // Debugging log
  
    const token = localStorage.getItem("token");
  
    try {
      // Make DELETE request to the API with the valid mainFieldId
      const response = await fetch(`${API_URL}/field/delete/mainfield/${mainFieldId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        alert("✅ ลบสนามหลักและสนามย่อยทั้งหมดสำเร็จ");
        // Optionally update the UI or remove the field from the list
      } else {
        alert("❌ เกิดข้อผิดพลาดในการลบสนามหลัก");
      }
    } catch (error) {
      console.error("❌ Error deleting main field:", error);
    }
  };
  ...
  const handleDeleteClick = (subField) => {
    console.log(subField.sub_field_id); // This should log the correct sub_field_id
  ...
  const deleteSubField = async (sub_field_id) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`❌ ${errorData.error || "เกิดข้อผิดพลาดในการลบสนาม"}`);
      }
    } catch (error) {
      console.error("❌ Error deleting sub-field:", error);
      alert("❌ Error deleting sub-field");
    }
  };
        ...
        {editingField === "documents" ? (
          ...
          <>
            ...
            <div className="field-card">
              <button className="delete-button" onClick={deleteMainField}>
                ลบสนามหลัก
              </button>
            </div>
          </>
          ...
          {editingField === sub.sub_field_id ? (
            ...
            <>
              ...
              {subFields.map((subField) => (
                <div key={subField.sub_field_id}>
                  <span>{subField.name}</span>
                  <button onClick={() => handleDeleteClick(subField)}>
                    ลบสนามย่อย
                  </button>
                </div>
              ))}
                  ...
                  <li key={`${sub.sub_field_id}-${addon.add_on_id}`}>
                    {editingAddon.addOnId === addon.add_on_id ? (
                      ...
                      <>
                        ...
                        <button onClick={() => deleteAddOn(addon.add_on_id)}>
                          ลบ Add-On
                        </button>
                      </>
                    )}
