...
export default function CheckFieldDetail() {
  ...
  const saveSubField = async (sub_field_id) => {
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`${API_URL}/field/supfiled/${sub_field_id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        ...
        setSubFields((prevSubFields) =>
          prevSubFields.map((sub) =>
            sub.sub_field_id === sub_field_id
              ? { ...sub, sub_field_name: updatedSubFieldName, price: updatedPrice, sport_id: updatedSportId }
              : sub
          )
        );
        cancelEditing();
  ...
  const saveAddon = async () => {
    try {
      const response = await fetch(`${API_URL}/add_on/${editingAddon.addOnId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          prevSubFields.map((sub) => ({
            ...
            add_ons: sub.add_ons.map((addon) =>
              addon.add_on_id === editingAddon.addOnId
                ? { ...addon, content: editingAddon.content, price: editingAddon.price }
                : addon
            ),
          }))
        );
  ...
  <div key={sub.sub_field_id} className="sub-field-card">
    ...
    {editingField === sub.sub_field_id ? (
      <>
        ...
        <button onClick={() => saveSubField(sub.sub_field_id)}>💾 บันทึก</button>
        <button onClick={() => cancelEditing()}>❌ ยกเลิก</button>
      </>
    ) : (
      <>
        ...
        <button onClick={() => startEditingSubField(sub)}>✏️ แก้ไข</button>
      </>
    )}
