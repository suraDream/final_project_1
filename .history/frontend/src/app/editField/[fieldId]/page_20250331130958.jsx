...
export default function CheckFieldDetail() {
  ...
  const saveDocumentField = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setField({
          ...field,
          documents: result.paths || selectedFile.map((file) => file.name).join(", "),
        });
        setEditingField(null);
        setSelectedFile(null);
      } else {
  ...
  const saveField = async (fieldName) => {
    try {
      if (fieldName === "img_field" || fieldName === "documents") {
        ...
        if (fieldName === "img_field") {
          // หากเป็นไฟล์รูปภาพ, ส่งไฟล์เดียว
          formData.append(fieldName, selectedFile[0]);
        } else {
          // หากเป็นไฟล์เอกสาร, ส่งหลายไฟล์
          for (let i = 0; i < selectedFile.length; i++) {
            formData.append(fieldName, selectedFile[i]);
          }
        }
  ...
  {editingField === "img_field" ? (
    <div className="preview-container">
      ...
      <button className="savebtn" onClick={saveImageField}>บันทึก</button>
  ...
  {editingField === "documents" ? (
    <>
      <input
        ...
        accept="image/*,.pdf"
      />
      <button className="savebtn" onClick={saveDocumentField}>บันทึก</button>
      <button className="canbtn" onClick={cancelEditing}>ยกเลิก</button>
    </>
  ) : (
    <>
