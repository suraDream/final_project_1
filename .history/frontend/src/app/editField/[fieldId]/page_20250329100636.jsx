...
export default function CheckFieldDetail() {
  ...
  const deleteMainField = async (fieldId) => {
    ...
    if (!mainFieldId) {
      console.error("❌ Invalid main field ID");
      return <div>Error: Invalid ID</div>;
    }
    console.log("Main Field ID:", mainFieldId);
  // ตรวจสอบว่า main_field_id เป็นตัวเลข
  ...
  try {
    const response = await fetch(`${API_URL}/field/delete/mainfield/${fieldId}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    if (response.ok) {
      ...
      // อัพเดต UI หรือทำการลบสนามหลักจากรายการ
    } else {
