...
export default function CheckFieldDetail() {
  ...
  const handleFileChange = (e) => {
    ...
    if (isValid) {
      setSelectedFile(files); // If multiple files are selected, this will not work well, consider handling them individually
      setUpdatedValue(files[0].name); // Update with first file name
    } else {
      e.target.value = null; // Reset the file input if the validation failed
    }
  };
