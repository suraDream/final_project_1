...
const saveField = async (fieldName) => {
  try {
    if (fieldName === "img_field" || fieldName === "documents") {
      ...
      // เพิ่มไฟล์ทั้งหมดใน selectedFile ไปยัง formData
      for (let i = 0; i < selectedFile.length; i++) {
        formData.append(fieldName, selectedFile[i]);  // ส่งไฟล์หลายไฟล์
      }
      ...
      if (response.ok) {
        ...
        setMessageType("success");
        setField({ ...field, [fieldName]: result.paths || selectedFile.map(file => file.name).join(", ") });
        setEditingField(null);
