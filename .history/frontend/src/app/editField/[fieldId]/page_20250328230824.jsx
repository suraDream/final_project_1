...
export default function CheckFieldDetail() {
  ...
  const saveSubField = async (sub_field_id) => {
    if (!updatedSportId) {
      ...
      return; // Prevent saving if no sport is selected
    }
    const token = localStorage.getItem("token");
    try {
  ...
  const handleAddSubField = async () => {
    if (!newSportId || isNaN(newSportId)) {
      alert("❌ กรุณาเลือกประเภทกีฬาก่อนเพิ่มสนาม");
      return; // Prevent adding if no sport is selected or sport_id is invalid
    }
  
    // Proceed with sending the request to add the sub-field
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`${API_URL}/field/subfield/${field_id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({
          sub_field_name: newSubFieldName,
          price: newPrice,
          sport_id: newSportId, // Ensure sport_id is a valid integer
          user_id: userId,
        }),
      });
      ...
      if (response.ok) {
        ...
        setSubFields((prevSubFields) => [
          ...prevSubFields,
          { sub_field_name: newSubFieldName, price: newPrice, sport_id: newSportId },
        ]);
        resetForm();
      } else {
        alert("❌ เกิดข้อผิดพลาดในการเพิ่มสนามย่อย");
      }
    } catch (error) {
      console.error("❌ Error adding sub-field:", error);
    }
  };
