...
console.log("fieldId from useParams:", fieldId);
  }, []);
  ...
  const deleteMainField = async (fieldId) => {
    // ถ้า fieldId เป็นออบเจ็กต์ ให้ดึง field_id ออกมาเป็นตัวเลข
    const mainFieldId = Array.isArray(fieldId) ? fieldId[0] : fieldId;
    const idToDelete = mainFieldId?.field_id || mainFieldId;
  
    // ตรวจสอบว่าค่าที่ดึงมาเป็นตัวเลขหรือไม่
    if (!idToDelete || isNaN(idToDelete)) {
      alert("❌ ID ของสนามหลักไม่ถูกต้อง");
      return;
    }
  
    console.log("Main Field ID:", idToDelete);
  
    const token = localStorage.getItem("token");
  
    try {
      const response = await fetch(`${API_URL}/field/delete/mainfield/${idToDelete}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
