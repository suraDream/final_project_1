...
export default function CheckFieldDetail() {
  ...
  const addSubField = async (userId) => {
    ...
    if (response.ok) {
      ...
      setMessage("เพิ่มสนามย่อยสำเร็จ") // Update the subFields state with the new sub-field
    } else {
  ...
  const saveAddon = async () => {
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        setMessage("อัปเดต Add-on สำเร็จ");
        setMessageType("success");
        setSubFields((prevSubFields) =>
          prevSubFields.map((sub) => ({
            ...sub,
        ...
        setMessage("เกิดข้อผิดพลาดในการอัปเดต Add-on");
        setMessageType("error");
      }
    } catch (error) {
