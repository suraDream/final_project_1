...
export default function CheckFieldDetail() {
  ...
  const handleFileChange = (e) => {
    const files = e.target.files[0];  // แก้ไขเป็นหลายไฟล์
    let isValid = true;
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      // ตรวจสอบให้แน่ใจว่าเป็นไฟล์รูปภาพหรือ PDF
    ...
    if (isValid) {
      ...
      setUpdatedValue(files.name);  // อัปเดตชื่อไฟล์ (ใช้ไฟล์แรกในกรณีที่เลือกหลายไฟล์)
    } else {
      e.target.value = null;  // รีเซ็ตค่าเมื่อไม่ผ่านการตรวจสอบ
    }
  };
