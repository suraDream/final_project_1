...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    // ตรวจสอบฟิลด์ทุกฟิลด์ว่ามีข้อมูลหรือไม่
    ...
    // ตรวจสอบอีเมล และ ชื่อผู้ใช้แบบ Real-Time
    if (!newErrors.user_name && !newErrors.email) {
      try {
        const response = await Promise.all([
          fetch(`${API_URL}/register/check-duplicate?field=user_name&value=${formData.user_name}`),
          fetch(`${API_URL}/register/check-duplicate?field=email&value=${formData.email}`)
        ]);
  
        const [userNameData, emailData] = await Promise.all([response[0].json(), response[1].json()]);
        ...
        console.error("Error checking duplicates:", error);
        newErrors.serverError = 'เกิดข้อผิดพลาดระหว่างการตรวจสอบอีเมล/ชื่อผู้ใช้';
      }
    }
    ...
    // ถ้าไม่มีข้อผิดพลาด ส่งข้อมูลฟอร์มไปที่ API
    try {
