...
export default function AdminManager() {
  ...
  const deleteField = async (fieldId) => {
    if (!window.confirm("คุณแน่ใจหรือไม่ว่าต้องการลบสนามกีฬานี้?")) return;
    ...
    try {
      ...
      if (response.ok) {
        setMessage("สนามกีฬาถูกลบเรียบร้อย"); // กำหนดข้อความเมื่อลบสำเร็จ
        setMessageType("success"); // กำหนดประเภทข้อความเป็น success

        // ✅ อัปเดต state ให้ลบสนามออกจากรายการที่แสดง
        setAllowFields(
          allowFields.filter((field) => field.field_id !== fieldId)
        );
      } else {
        setMessage(`เกิดข้อผิดพลาด: ${data.error}`); // กำหนดข้อความข้อผิดพลาด
        setMessageType("error"); // กำหนดประเภทข้อความเป็น error
      }
    } catch (error) {
      console.error("❌ Error deleting field:", error);
      setMessage("เกิดข้อผิดพลาดในการลบสนามกีฬา"); // กำหนดข้อความเมื่อเกิดข้อผิดพลาด
      setMessageType("error"); // กำหนดประเภทข้อความเป็น error
    }
  };
