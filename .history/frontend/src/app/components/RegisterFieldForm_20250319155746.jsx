...
export default function RegisterFieldForm() {
  ...
  const handleFieldChangeDeposit = (e) => {
    ...
    // ตรวจสอบว่าไม่ใช่ค่าว่างหรือไม่
    const numericValue = value === "" ? "" : Math.abs(parseFloat(value)); // ป้องกันค่าลบ
    setFieldData({ ...fieldData, [name]: numericValue });
  };
  
  const handleCheckboxChange = (e) => {
    const { name, checked } = e.target;
    setFieldData({ ...fieldData, [name]: checked }); // ตั้งค่า depositChecked เป็น true หรือ false
  };
  ...
  <input
    type="checkbox"
    name="depositChecked"
    checked={fieldData.depositChecked} // ค่าที่ใช้ในการตรวจสอบว่าผู้ใช้เลือกหรือไม่
    onChange={handleCheckboxChange} // ฟังก์ชันในการจัดการการเลือก
  />
</div>
...
{fieldData.depositChecked && (
 ...
 <input
   type="number"
   name="price_deposit"
   placeholder="ถ้าไม่ได้เก็บให้ว่างไว้"
   value={fieldData.price_deposit || ""} // ตรวจสอบว่าเป็น "" ถ้าเป็น undefined หรือ null
