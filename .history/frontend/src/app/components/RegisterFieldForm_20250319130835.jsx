...
export default function RegisterFieldForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const storedUser = localStorage.getItem("user");
    if (!storedUser) {
      setMessage("กรุณาเข้าสู่ระบบก่อน!"); // ใช้ setMessage แทน alert
      return;
    }
    ...
    if (!fieldData.field_name || !fieldData.address || !fieldData.gps_location || !fieldData.open_hours || !fieldData.close_hours || !fieldData.number_bank || !fieldData.account_holder) {
      setMessage("กรุณากรอกข้อมูลให้ครบถ้วน"); // แสดงข้อความ error
      return;
    }
  
    // ถ้าเลือกเอกสารหรือรูปภาพไม่ได้
    if (!fieldData.documents || !fieldData.img_field) {
      setMessage("กรุณาเลือกเอกสารและรูปโปรไฟล์สนาม"); // แสดงข้อความ error
      return;
    }
  
    const formData = new FormData();
    formData.append("documents", fieldData.documents[0]); // แนบเอกสาร
    ...
    try {
      ...
      if (data.error) {
        console.error("Error:", data.error);
        setMessage("เกิดข้อผิดพลาด: " + data.error); // แสดงข้อความ error
        return;
      }
  
      setMessage("ลงทะเบียนสนามเรียบร้อย!"); // แสดงข้อความ success
      setFieldData({
        field_name: "",
        address: "",
        gps_location: "",
      ...
      setMessage("เกิดข้อผิดพลาดในการส่งข้อมูล"); // แสดงข้อความ error
    }
  };
