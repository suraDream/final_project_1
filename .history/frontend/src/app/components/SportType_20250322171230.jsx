...
export default function HomePage() {
    ...
    fetch(`${API_URL}/sports_types/preview`, {
      ...
      .then((data) => {
        if (data.error) {
          ...
          setApprovedFields(data); // Set the categories if no error
        }
      })
      .catch((error) => {
  ...
  const convertToThaiDays = (days) => {
    if (!days) return ""; // Return an empty string if `days` is undefined or null
    ...
    // If `days` is a string, split it and map each day to Thai
    return days
      .split(" ")
          ...
          <div className="grid">
            {approvedFields.length > 0 ? (
              approvedFields.map((field, index) => (
                <div
                  key={`${field.field_id}-${index}`} // Combine field_id with index to ensure uniqueness
                  className="card"
                ...
                >
                  ...
                  <div className="card-body">
                    ...
                    <div className="firstopen">
                      ...
                      <h3>{field.field_name}</h3>
                      <p>ประเภทกีฬา: {field.sport_name}</p>
                    </div>
                  </div>
                </div>
              ))
