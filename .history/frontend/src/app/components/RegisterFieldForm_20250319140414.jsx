...
export default function RegisterFieldForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    for (const facId of selectedFacs) {
      // ตรวจสอบว่าได้กรอกราคาให้สิ่งอำนวยความสะดวกนั้น ๆ หรือไม่
      if (selectedFacilities[facId] === "") {
        const fac = facilities.find((f) => f.fac_id === facId); // ค้นหาข้อมูลชื่อสิ่งอำนวยความสะดวก
        console.log('Selected facId:', facId); // ตรวจสอบ facId ที่เลือก
        console.log('All facilities:', facilities); // ตรวจสอบข้อมูลทั้งหมดของ facilities
  
        if (fac) { // ตรวจสอบว่า fac ไม่เป็น undefined
          alert(`กรุณากรอกราคาสำหรับสิ่งอำนวยความสะดวก: ${fac.fac_name}`); // แสดงชื่อสิ่งอำนวยความสะดวก
        } else {
          alert(`ไม่พบข้อมูลสิ่งอำนวยความสะดวกกับ ID: ${facId}`); // แสดงว่าไม่พบข้อมูล
        }
        return;
      }
    }
          ...
          <input
            ...
            onBlur={() => {
              ...
              if (
                ...
                setMessage("เลขที่กรอกไม่ถูกต้อง กรุณากรอกให้ตรงกับเงื่อนไข");
