...
export default function RegisterFieldForm() {
  ...
  const handleFieldChangeDeposit = (e) => {
    ...
    // ตรวจสอบค่าที่กรอกและแปลงเป็นค่าบวกหรือ 0
    const numericValue = Math.abs(parseFloat(value)); // แปลงค่าที่กรอกให้เป็นค่าบวกเสมอ

    setFieldData({ ...fieldData, [e.target.name]: numericValue });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      console.log("ข้อมูลที่กรอกไม่ครบ:");
    console.log("Field Name:", fieldData.field_name);
    console.log("Address:", fieldData.address);
    console.log("GPS Location:", fieldData.gps_location);
    console.log("Open Hours:", fieldData.open_hours);
    console.log("Close Hours:", fieldData.close_hours);
    console.log("Number Bank:", fieldData.number_bank);
    console.log("Account Holder:", fieldData.account_holder);
    console.log("Price Deposit:", fieldData.price_deposit);
    console.log("Name Bank:", fieldData.name_bank);
    ...
    for (let sub of subFields) {
      if (!sub.name || !sub.price || !sub.sport_id) {
        alert("กรุณากรอกข้อมูลให้ครบถ้วนสำหรับสนามย่อยทุกสนาม");
        return;
      }
    }
      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        <div className="input-group">
          ...
          <input
            type="number"
            name="price_deposit"
            placeholder="ถ้าไม่ได้เก็บใส่ 0 "
            value={fieldData.price_deposit}
            onChange={(e) => handleFieldChangeDeposit(e)}
            onKeyDown={(e) => {
              // ป้องกันการกรอกเครื่องหมายลบ
              if (e.key === "-") {
                alert("ห้ามใส่ - ");
                e.preventDefault(); // ป้องกันไม่ให้กรอกเครื่องหมายลบและตัวอักษร e
              }
            }}
          />
        </div>
