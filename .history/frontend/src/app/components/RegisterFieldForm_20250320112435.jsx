...
export default function RegisterFieldForm() {
  ...
  // ฟังก์ชันในการแปลงวันจากภาษาไทยเป็นภาษาอังกฤษก่อนส่งไปยังฐานข้อมูล
  const convertToEnglish = (daysInThai) => {
    return daysInThai.map((day) => dayMapping[day]);
  };
  
  // ตัวอย่างการใช้ใน onChange เมื่อเลือกวันเปิดบริการ
  const handleCheckboxChangeLang = (e) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const daysInEnglish = convertToEnglish(fieldData.open_days);
    ...
    formData.append("open_days", JSON.stringify(daysInEnglish));
    formData.append(
      "data",
      JSON.stringify({
        user_id: userId,
    ...
    try {
      ...
      alert("ลงทะเบียนสนามเรียบร้อยรอผู้ดูแลระบบตรวจสอบ");
      ...
      setTimeout(() => {
        setMessage("");
        window.location.reload(); // หรือใช้ router.push() ถ้าใช้ Next.js เพื่อเปลี่ยนหน้า
      }, 2000); // รีเฟรชหลังจาก 2 วินาที
    } catch (error) {
                  ...
                  {[
                    ...
                    <label key={index} className="checkbox-label">
                      <input
                        ...
                        name="open_days"
                        value={day}
                        onChange={handleCheckboxChangeLang} // เรียกใช้ฟังก์ชันการจัดการ
                      />
                      {day}
                    </label>
                  ))}
