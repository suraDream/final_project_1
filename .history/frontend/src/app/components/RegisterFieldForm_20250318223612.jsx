...
export default function RegisterFieldForm() {
  ...
  const addSubField = () => {
    ...
    setSubFields([...subFields, { name: "", price: "", sport_id: "", user_id: userId, addOns: [] }]); 
  };
...
{subFields.map((sub, subIndex) => (
  <div key={subIndex} style={{ border: "1px solid #ccc", padding: "10px", marginBottom: "10px" }}>
    ...
    <input
      ...
      placeholder="ราคา"
      value={sub.price}
      onChange={(e) => updateSubField(subIndex, "price", e.target.value)}
    />

    {/* ✅ Dropdown เลือกประเภทกีฬา */}
    <select
      value={sub.sport_id}
      onChange={(e) => updateSubField(subIndex, "sport_id", e.target.value)}
    >
    ...
    {sub.addOns.map((addon, addOnIndex) => (
      <div key={addOnIndex} style={{ marginLeft: "20px", marginTop: "5px" }}>
        ...
        <input
          ...
          value={addon.content}
          onChange={(e) => updateAddOn(subIndex, addOnIndex, "content", e.target.value)}
        />
        ...
        <input
          ...
          placeholder="ราคา"
          value={addon.price}
          onChange={(e) => updateAddOn(subIndex, addOnIndex, "price", e.target.value)}
        />

        {/* ✅ ปุ่มลบกิจกรรมเพิ่มเติม */}
        <button type="button" onClick={() => removeAddOn(subIndex, addOnIndex)}>
          ลบกิจกรรมเพิ่มเติม
        </button>
      </div>
    ))}
