...
export default function AdminPage() {
  ...
  const deleteFacility = async (facId) => {
    const res = await fetch(`${API_URL}/facilities/delete/${facId}`, {
      method: "DELETE",
    });

    if (res.ok) {
      setFacilities(facilities.filter((facility) => facility.fac_id !== facId));
    } else {
      alert("ไม่สามารถลบสิ่งอำนวยความสะดวกได้");
    }
  };

  // ฟังก์ชันลบประเภทกีฬา
  const deleteSport = async (sportId) => {
    const res = await fetch(`${API_URL}/sports_types/delete/${sportId}`, {
      method: "DELETE",
    });
    ...
    if (res.ok) {
      ...
      alert("ไม่สามารถลบประเภทกีฬาได้");
    }
  };
        ...
        <div className="factcon">
          {facilities.map((fac) => (
            <div key={fac.fac_id} className="facility-item">
              ...
              <button onClick={() => deleteFacility(fac.fac_id)} className="delete-btn">
                ลบ
              </button>
            </div>
          ))}
        ...
        <div className="sports-list">
          {sports.map((sport) => (
            <div key={sport.sport_id} className="sport-item">
              ...
              <button onClick={() => deleteSport(sport.sport_id)} className="delete-btn">
                ลบ
              </button>
            </div>
          ))}
