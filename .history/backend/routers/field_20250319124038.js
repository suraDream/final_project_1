...
router.post("/register", upload.fields([{ name: "documents" }, { name: "img_field" }]), async (req, res) => {
  try {
    ...
    const documents = req.files["documents"] ? req.files["documents"][0].path : null;
    const imgField = req.files["img_field"] ? req.files["img_field"][0].path : null;
...
for (const sub of subFields) {
  ...
  const sub_field_id = subFieldResult.rows[0].sub_field_id;
   ...
   for (const addon of sub.addOns) {
    ...
    res.json({
      message: "สนามกีฬาได้รับการลงทะเบียนแล้ว",
