...
router.delete("/delete/addon/:id", async (req, res) => {
  ...
  // Verify the user's token (You can use a JWT middleware here)
  if (!token) {
  ...
  try {
    ...
    const addOnQuery = await pool.query(
      "SELECT * FROM addons WHERE add_on_id = $1",
      [addOnId]
    );
    ...
    await pool.query("DELETE FROM addons WHERE add_on_id = $1", [addOnId]);
