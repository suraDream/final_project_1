...
router.get("/", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT user_id, user_name, first_name, last_name, email, role FROM users"
    );
...
router.put("/:id", authMiddleware, async (req, res) => {
  ...
  try {
    // ✅ ตรวจสอบสิทธิ์: อนุญาตเฉพาะ Admin หรือ เจ้าของบัญชีเองเท่านั้น
  ...
  try {
    ...
    if (!first_name || !last_name) {
      return res
        .status(400)
        .json({
          message: "ข้อมูลไม่สมบูรณ์: ชื่อหรือานามสกุลไม่สามารถเป็นค่าว่าง",
        });
    }
    ...
    const updateResult = await pool.query(
      "UPDATE users SET password = $1, first_name = $2, last_name = $3, email = $4, role = $5 WHERE user_id = $6",
      [hashedPassword, first_name, last_name, email, role, id]
    );
