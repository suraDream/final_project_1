...
router.put("/:field_id", async (req, res) => {
  try {
    const { field_id } = req.params;
    const { field_name, address, gps_location, open_hours, close_hours, price_deposit, name_bank, account_holder, number_bank, status } = req.body;
    ...
    // ✅ อัปเดตข้อมูลสนามกีฬา พร้อมกับอัปเดตสถานะ
    console.log("📌 กำลังอัปเดตสนามกีฬา:", {
      field_name, address, gps_location, open_hours, close_hours, price_deposit, name_bank, account_holder, number_bank, field_id, status
    });
    ...
    const result = await pool.query(
      `UPDATE field 
       SET field_name = COALESCE($1, field_name), 
           ...
           status = COALESCE($10, status)  -- เพิ่มการอัปเดตสถานะ
       WHERE field_id = $11 
       RETURNING *;`,
      [field_name, address, gps_location, open_hours, close_hours, price_deposit, name_bank, account_holder, number_bank, status, field_id]
    );
