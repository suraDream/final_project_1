...
router.get("/:field_id", authMiddleware, async (req, res) => {
  try {
    ...
    const user = jwt.decode(token);  // ใช้ JWT decode เพื่อดึงข้อมูลของผู้ใช้

    // ตรวจสอบว่าเป็น admin หรือ field_owner
    if (user.role === "admin") {
        ...
        `SELECT 
          ...
          COALESCE(json_agg(DISTINCT jsonb_build_object(
            ...
            'add_ons', (
              ...
              FROM add_on a WHERE a.sub_field_id = s.sub_field_id
            )
          )), '[]'::json) AS sub_fields
        FROM field f
    ...
    // ตรวจสอบว่าเป็น field_owner
    if (user.role === "field_owner" && user.fieldId === field_id) {
      const result = await pool.query(
        `SELECT 
          f.field_id, f.field_name, f.address, f.gps_location, f.documents,
          f.open_hours, f.close_hours, f.img_field, f.name_bank, 
    ...
    // หาก role ไม่ใช่ admin หรือ field_owner หรือไม่ตรงกับ fieldId
...
router.put("/edit/:field_id", authMiddleware, async (req, res) => {
  try {
    ...
    // ตรวจสอบว่าเป็น admin หรือ field_owner
    if (user.role === "admin") {
      // admin สามารถอัปเดตข้อมูลทุกฟิลด์
      const result = await pool.query(
        `UPDATE field 
    ...
    // ตรวจสอบว่า field_owner สามารถอัปเดตข้อมูลของตนเองได้หรือไม่
    if (user.role === "field_owner" && user.fieldId === field_id) {
      const result = await pool.query(
        `UPDATE field 
         SET field_name = COALESCE($1, field_name), 
    ...
    console.error("❌ Database Error:", error);
    res.status(500).json({ error: "เกิดข้อผิดพลาดในการอัปเดตข้อมูลสนามกีฬา" });
  }
});
