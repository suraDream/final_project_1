...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const updateResult = await pool.query("UPDATE users SET first_name = $1, last_name = $2, email = $3, role = $4 WHERE user_id = $5", [
      first_name,
      last_name,
      email,
      role,
    ...
    res.status(200).json({ message: "เปลี่ยนข้อมูลโปรไฟล์เรียบร้อยแล้ว" });
  } catch (error) {
