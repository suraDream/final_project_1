...
router.get("/:field_id", authMiddleware, async (req, res) => {
  try {
    ...
    const { user_id } = req.user;  // ใช้ user_id ที่ได้จากการตรวจสอบ token ใน middleware
    ...
    const result = await pool.query(
      `SELECT 
        ...
        WHERE f.field_id = $1 AND u.user_id = $2
        GROUP BY f.field_id, u.user_id;
        `,
      [field_id, user_id]  // ใช้ user_id จาก token หรือ session เพื่อเช็คว่าเป็นเจ้าของฟิลด์นี้จริง
    );
