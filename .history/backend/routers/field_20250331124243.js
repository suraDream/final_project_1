...
// เปลี่ยนจาก .single() เป็น .array() สำหรับการอัปโหลดหลายไฟล์
router.post("/:field_id/upload-image", authMiddleware, upload.single("img_field", 10), async (req, res) => {
  try {
    const { field_id } = req.params;
    ...
    res.json({ message: "✅ อัปโหลดรูปสำเร็จ", paths: filePaths });
  } catch (error) {
