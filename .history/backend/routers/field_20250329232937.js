...
router.post("/:field_id/upload-image",authMiddleware, upload.single("img_field"),authMiddleware, async (req, res) => {
  try {
    const { field_id } = req.params;
...
router.post("/:field_id/upload-document", upload.single("documents"),authMiddleware, async (req, res) => {
  try {
    const { field_id } = req.params;
    const filePath = req.file?.path;
