...
// Home api /////////////////////////////////////////////////////////////////////////////////////////////////////////
router.get('/allow/preview', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT 
        field.field_id,
      ...
      FROM field
      WHERE status = 'ผ่านการอนุมัติ'
      ORDER BY field_id ASC LIMIT 10
    `);
    res.json(result.rows);
  } catch (error) {
    console.error('Error:', error);
...
router.get('/sports-categories', async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT sport_id, sport_name FROM sports_types');
    res.json(result.rows);
  } catch (error) {
    console.error('Database Error:', error);
    res.status(500).json({ error: 'Database error fetching sports categories' });
  }
});
