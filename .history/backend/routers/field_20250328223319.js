...
router.put("/supfiled/:sub_field_id", async (req, res) => {
  ...
  try{
    ...
    await pool.query(`UPDATE sub_field SET sub_field_name = $1,price = $2, sport_id =$3 WHERE sub_field_id=$4`
...
router.put("/add_on/:add_on_id",async(req,res)=>{
   ...
   try{
      ...
      await pool.query(`UPDATE add_on SET content = $1, price = $2 WHERE add_on_id = $3`, [content, price, add_on_id]);
