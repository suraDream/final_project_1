...
router.get("/myfields", authMiddleware, async (req, res) => {
  try {
    ...
    try {
      const result = await client.query(query, [user_id]);  // ดึงข้อมูลจากฐานข้อมูล
      ...
      client.release();  // ปล่อยการเชื่อมต่อฐานข้อมูล
      res.json(result.rows);  // ส่งข้อมูลกลับไปที่ client
    } catch (queryError) {
