...
router.post("/:field_id/upload-image", authMiddleware, upload.fields([{ name: "img_field" }, { name: "documents" }]), async (req, res) => {
  try {
    const { field_id } = req.params;
    ...
    await pool.query(
      `UPDATE field SET img_field = $1, documents = $2 WHERE field_id = $3`,
      [imgField, documents, field_id]
    );

    res.json({ message: "✅ อัปโหลดไฟล์สำเร็จ", imgField, documents });
  } catch (error) {
    console.error("❌ Upload error:", error);
    res.status(500).json({ error: "❌ อัปโหลดไฟล์ไม่สำเร็จ", details: error.message });
  }
});
